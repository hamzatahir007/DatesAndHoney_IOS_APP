import { ActivityIndicator, Dimensions, Image, Modal, Platform, SafeAreaView, ScrollView, StatusBar, StyleSheet, Text, ToastAndroid, TouchableOpacity, View } from 'react-native';
...
const ProfileScreen = ({ navigation }) => {
  ...
  const uploadImage = async (tempImage, names) => {
    if (tempImage == null) {
      Toast.show('Network issue please try again!!', Toast.SHORT)
      return null;
    }
    setModal(true)
      ...
      try {
        ...
        if (url) {
          ...
          useRef.update({
            ...
            Toast.show('image uploaded successfull', Toast.SHORT)
            setModal(false)
  ...
  const pickImage1 = async () => {
    ...
    setUploading(true)
    // const uploadUri = Platform.OS === 'ios' ? tempImage.replace('file://', '') : tempImage;

    // console.log(tempImage , uploadUri);
    // return
              ...
              <TouchableOpacity
                ...
                <View style={{
                  ...
                  <Text style={{
                    ...
                    {walletAmount?.totalBalance ? `$${walletAmount?.totalBalance}` : '$0.00'}
                  </Text>
                </View>
              </TouchableOpacity>
            </View>
