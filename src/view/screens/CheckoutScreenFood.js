...
const CheckoutScreenFood = ({ navigation, route }) => {
    // const {data}= route?.params;
    const AddToCard = useSelector(selectaddToCart)
    // const [AddToCard, setAddToCard] = useState(data);
            ...
            if (!AddToCard?.length == 0) {
                ...
                if (totalBalance >= price) {
                    ...
                    try {
                        ...
                        let walletData = {
                            expend: foodmenuPricing?.totalPriceWithTax ? foodmenuPricing?.totalPriceWithTax :
                                foodmenuPricing?.totalPrice,
                            fare: 0,
                            currency: 'usd',
                            deposit: 0,
                            type: 'purchase foods',
                        ...
                        await firestore()
                            ...
                            .then(async () => {
                                ...
                                const orderDetails = {
                                    ...
                                    DateAndHoneyFee: foodmenuPricing?.DateAndHoneyFee ? foodmenuPricing?.DateAndHoneyFee : '0',
                                    transactionFee: foodmenuPricing?.transactionFee ? foodmenuPricing?.transactionFee : '0',
                                    totalPricewithTaxes: foodmenuPricing?.totalPriceWithTax,
                                    CouponCode: foodmenuPricing?.CouponCode,
                                    CouponCodeutilized: foodmenuPricing?.CouponCodeutilized,
                                    status: true,
                                };
                                ...
                                Toast.show('Tickets Purchase successfully', Toast.SHORT);
                                setUploading(false);
                                ...
                                setFoodOrderContent({
                                    ...
                                    qr: ids ? ids + '-' + user?.uid : user?.uid + '-' + ids,
                                })
                                //     await docRef.set({
                                //         TicketsHistory: [
                                //             {
                ...
                //         })
                //         .then(() => {
                //             Toast.show('Your Food Order Successfully Placed', Toast.SHORT)
                //             dispatch(addToCart(''))
                    ...
                    await firestore()
                        ...
                        .then(async () => {
                            ...
                            useRef.update({
                                ...
                                Toast.show(`${MembershipName} Membership hase been Purchased successfully`, Toast.SHORT)
                                // setLoading(true)
                                setShowModalContent({
                                    ...showModalContent,
    ...
    const PayFlakeBillAmount = async () => {
        ...
        if (SelectedPaymentCards?.length > 0) {
            if (FlakeBill) {
                        ...
                        await firestore()
                            ...
                            .then(async () => {
                                ...
                                userRef.update({
                                    ...
                                    // Toast.show(`Successfully: ${FlakeBill?.Flake} Flakes remove from your profile!!`, Toast.SHORT)
                ...
                setUploading(false)
                Toast.show(`Network error please try agin!`, Toast.SHORT)
            }
        }
        else {
            Toast.show(`Please enter your payment card first`, Toast.LONG);
                ...
                try {
                    ...
                    await firestore()
                        ...
                        .then(async () => {
                            ...
                            Toast.show(`Additional Package for ${AdditionalPackag?.Title} hase been Purchased successfully`, Toast.SHORT)
                            setUploading(false)
                            setShowModalContent({
                                ...showModalContent,
                                title: `${AdditionalPackag?.Title}`,
            ...
            try {
                await firestore()
                    ...
                    .then(() => {
                        ...
                        Toast.show("Payment card added successfully!", Toast.SHORT);
                    })
            }
            catch (e) {
                setUploading(false)
    ...
    const onApplyCouponThree = async () => {
        ...
        // returnitem?.Foodid[0]?.Eventid == AddToCard[0]?.Eventid
        if (!addToCart?.appliedPromo || addToCart?.appliedPromo == '') {
            try {
                const querySnapshot = await firestore().collection('orders').doc(user?.uid).get();
                if (querySnapshot?.empty) {
                    console.log('empty');
                    setUploadingTwo(false)
                } else {
                    const ticketsHistory = querySnapshot.data()?.OrdersHistory || [];
                    if (ticketsHistory.some(
                        (item) =>
                            item?.OrderDetails[0]?.Eventid == promoterId?.Foodid[0]?.Eventid &&
                            item?.CouponCodeutilized === true &&
                            item?.CouponCode === applycopun
                    )) {
                        setUploadingTwo(false);
                        Toast.show(
                    ...
                    if (promoterId?.discountPercent) {
                        ...
                        // setUploadingTwo(false);
                        if (CheckUserPackage) {
                            const totalAmount = foodmenuPricing?.totalPricewithTaxes ? foodmenuPricing?.totalPricewithTaxes :
                                foodmenuPricing?.totalPrice;
                            // membershipDiscount = parseFloat((totalAmount * 5 / 100)).toFixed(2);

                            const appliedPromo = parseFloat((totalAmount * promoterId?.discountPercent) / 100).toFixed(2);
                            let sum = (foodmenuPricing?.totalPrice - appliedPromo).toFixed(2);
                            // const promoterReward = parseFloat((data?.Tickets?.pricePerTicket * data?.promoterReward) / 100).toFixed(2);
                            ...
                            //     // menu: AddToCard
                            //     totalPrice: totalSum,
                            //     totalPricewithTaxes: totalSum,
                            //     transactionFee: 0,
                            //     DateAndHoneyFee: 0,
                            // };
                            setFoodmenuPricing(updateTwo)
                            // console.log('===>', updateTwo);
                            setUploadingTwo(false);
                            Toast.show('Success: Coupon code applied successfully.', Toast.SHORT);
                            return
                            // return
                        }
                        else if (!CheckUserPackage) {
                            setUploadingTwo(false)
                            Toast.show('Coupon code not applicable.', Toast.SHORT);
                            setErrorApplyCoupon(true);
                            return
                        }
                    } else {
                        Toast.show('Coupon code already applied', Toast.SHORT);
                        setUploadingTwo(false);
                        return
                    }
                    ...
                    //             setUploadingTwo(false)
                    //             Toast.show('Sorry, this coupon code has already been used. Please try using a different code.', Toast.SHORT);
                    //         }
                    //     });
                ...
                setUploadingTwo(false)
                Toast.show(`Error: ${e.message}`, Toast.SHORT);
            }
            return
        }
        else {
            setUploadingTwo(false)
            Toast.show(`Promo code already applied`, Toast.SHORT);
        }
    }

    const onApplyCoupon = async () => {
        const today = new Date()
        if (!AddToCard.length == 0 && foodmenuPricing) {
            ...
            if (applycopun) {
                ...
                if (allAffiliateCode?.length > 0) {
                    ...
                    allAffiliateCode?.map((item) => {
                        ...
                        //     item?.VipCode == applycopun &&
                        //     item?.type == 'food' &&
                        //     item?.Foodid[0]?.Eventid == AddToCard[0]?.Eventid
                        // );
                        // setUploadingTwo(false);
                        // return
                        if (
                    ...
                    setUploadingTwo(false)
                    Toast.show(`Coupon code not available.`, Toast.SHORT)
                    setErrorApplyCoupon(true)
                }
            }
            else {
                setUploadingTwo(false)
                Toast.show(`Coupon code cannot be empty!`, Toast.SHORT)
                setErrorApplyCoupon(true)
            }
        }
    }
                ...
                <ScrollView vertical showsVerticalScrollIndicator={false}>
                        ...
                        <View>
                            {!AddToCard.length == 0 && foodmenuPricing &&
                                <View>
                                    ...
                                    {foodmenuPricing?.transactionFee > 1 &&
                                        <View style={{
                                            flexDirection: 'row',
                                            paddingHorizontal: 20,
                                            paddingVertical: 10,
                                            ...
                                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                                ...
                                                <SVGNotify
                                                    ...
                                                    onPress={() => {
                                                        setTaxes(true);
                                                        setTaxesType('TransactionFee');
                                                    }}
                                                />
                                            </View>
                                            <Text style={{ color: COLORS.green, fontSize: 12 }}>
                                                ${foodmenuPricing?.transactionFee || '00'}
                                            </Text>
                                        </View>
                                    }
                                    {foodmenuPricing?.DateAndHoneyFee > 1 &&
                                        <View style={{
                                            flexDirection: 'row',
                                            paddingHorizontal: 20,
                                            paddingVertical: 10,
                                            ...
                                            <Text style={{ fontSize: 12, color: COLORS.green }}>
                                                ${foodmenuPricing?.DateAndHoneyFee || '00'}
                                            </Text>
                                        </View>
                                    }
                                    {foodmenuPricing?.appliedPromo &&
                                        <View style={{
                                            ...
                                            <View style={{
                                                ...
                                                <Text style={{ fontSize: 14, color: COLORS.gray }}>{foodmenuPricing?.appliedPromoPercent}%</Text>
                                                {/* <SVGNotify width={15} height={15} onPress={() => { setTaxes(true), setTaxesType('DAHFee') }} /> */}
                                            </View>
                                    ...
                                    <View style={{
                                        ...
                                        <Text style={{ fontSize: 14, color: COLORS.black, fontWeight: 'bold' }}>${foodmenuPricing?.totalPriceWithTax ? foodmenuPricing?.totalPriceWithTax :
                                            foodmenuPricing?.totalPrice}</Text>
                                    </View>
                                </View>
                            }
                    ...
                    <View style={{
                        ...
                        {!uploading == true ?
                            <>
                                {!AddToCard.length == 0 && foodmenuPricing &&
                                    <CustomeButton onpress={() => BuyFood()}
                                        title={`Pay Amounts $${foodmenuPricing?.totalPriceWithTax ? foodmenuPricing?.totalPriceWithTax :
                                            foodmenuPricing?.totalPrice}`} width={width / 1.2} />
                                }
                            </>
                            :
                            <View style={{
...
export default CheckoutScreenFood
