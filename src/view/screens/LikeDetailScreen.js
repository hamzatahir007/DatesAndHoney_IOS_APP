import { ActivityIndicator, Dimensions, FlatList, Platform, Image, Modal, SafeAreaView, ScrollView, StatusBar, StyleSheet, Text, ToastAndroid, TouchableOpacity, View } from 'react-native';
...
const LikeDetailScreen = ({ navigation }) => {
  ...
  const fetchPremiumUser = async () => {
    ...
    allusers.forEach((documentSnapshot) => {
      ...
      // return
      if (data.uid !== user?.uid && data?.Category == 'User' && data?.Gender != user?.Gender) {
        const updateData = {
          ...data,
          matchpercent: '50%',
        }
  ...
  const fetchPremiumUserTwo = async () => {
    ...
    allusers.forEach((documentSnapshot) => {
      const data = documentSnapshot.userDetails
      if (data.uid !== user?.uid && data?.Category == 'User' && user?.Relagion == data?.Relagion) {
        const updateData = {
          ...data,
          matchpercent: '85%',
        }
  ...
  const SelectedAdvanceFilter = (item) => {
    ...
    if (!FilterModaldata?.includes(item)) {
      if (user?.PackageId == 654) {
        ...
        Toast.show('You need to buy a Premium package before applay more then three advance filter!', Toast.SHORT);
      }
    }
    else {
      Toast.show('This filter already applied!', Toast.SHORT);
  ...
  const OnPremiumPress = (props) => {
    if (props == 1) {
      if (user?.PackageId == 654 || user?.PackageId == 456) {
        Toast.show('Please upgrade your membership package to send messages!', Toast.SHORT);
      }
      else {
        Toast.show('Please upgrade your membership package to send messages!', Toast.SHORT);
      }
      ...
      Toast.show('Please upgrade your membership package to like users', Toast.SHORT);
    }
  }
  ...
  const onLikeCard = async (item) => {
    ...
    await firestore()
      ...
      .then(() => {
        ...
        Toast.show(`Youre like send to ${item.Name}`, Toast.SHORT);
        ...
        if (querySnapshot.data()?.PrivateChat) {
          querySnapshot.data()?.PrivateChat?.map((j) => {
            ...
            if (j?.ChatuserDetails?.uid == item?.uid) {
              ...
              Toast.show(`Please Like them back before start chating.`, Toast.SHORT);
            }
          })
        }
      })
    ...
    if (userDoc.exists) {
      ...
      if (privateChat) {
        ...
        if (chatUserDetails) {
          ...
          if (itemDoc.exists) {
            ...
            if (itemPrivateChat) {
              ...
              if (likedUser) {
                ...
                Toast.show(`Please wait till your partner likes you back.`, Toast.SHORT);
                return;
              }
            }
            else {
          ...
          Toast.show(`Please send Like before starting a chat.`, Toast.SHORT);
          return;
        }
      }
    }
                  ...
                  {usersLikedYou.map((item, index) => (
                    <View key={index}
                      ...
                      <TouchableOpacity
                        ...
                        style={{
                          height: 200,
                          opacity: user?.PackageId == 654 || user?.PackageId == 456 || user?.PackageId == 123 ? 20 : 0.3,
                          // width: '100%',
                          borderRadius: 10,
                          backgroundColor: COLORS.white,
                          ...Platform.select({
            ...
            <View style={{
              ...
              {!uploadingPremium.one ?
                        ...
                        <View key={index}
                          ...
                          <TouchableOpacity
                            ...
                            style={{
                              ...
                              opacity: user?.PackageId == 654 || user?.PackageId == 456 ? 20 : 0.3,
                            }}>
                            <View>
                              <Image source={{ uri: item.image1 }} resizeMode='cover'
                                blurRadius={user?.PackageId == 654 || user?.PackageId == 456 ? 0 : 30}
                                style={{
                                  height: 150,
                                  width: '100%',
                                  borderRadius: 10,
                ...
                <View style={{
                  ...
                  <ActivityIndicator size={'small'} color={COLORS.main} animating={uploadingPremium.one} />
                </View>
              }
            </View>
                      ...
                      {usersFromAiThree.map((item, index) => (
                        <View key={index}
                          ...
                          <TouchableOpacity
                            ...
                            <View>
                              <Image source={{ uri: item.image1 }} resizeMode='cover'
                                blurRadius={user?.PackageId == 654 ? 0 : 30}
                                style={{
                                  height: 150,
                                  width: '100%',
                                  borderRadius: 10,
                            ...
                            <View style={{
                              ...
                              <TouchableOpacity
                                disabled={user?.PackageId == 654 ? false : true}
                                onPress={() => item?.liked ? Toast.show('Liked Sended', Toast.SHORT) : onLikeCard(item)}
                                style={{
                                  padding: 5,
                                  borderRadius: 20,
                                  borderWidth: 1,
