...
const SelectionOneQuestionMultipleSubstance = ({ navigation, route }) => {
  ...
  const onSmokeScreen = async () => {
    if (smokeData != null && vapeData != null && marijuanaData != null && drugsData != null && drinkData != null) {
      ...
      try {
        ...
        await firestore()
          .collection('Users').doc(CurrentUser).update({
            ...
            'userDetails.KosherType': data?.KosherType,
            'userDetails.Marijauna': data?.Marijauna,
            'userDetails.ParentReligion': data?.ParentReligion ? data?.ParentReligion : null,
            'userDetails.PoliticalView': data?.PoliticalView,
            'userDetails.Relagion': data?.Relagion,
            'userDetails.Smoke': data?.Smoke,
            'userDetails.Vape': data?.Vape,
            'userDetails.foodtype': data?.foodtype ? data?.foodtype : null,
            'userDetails.image1': imageUrl,
            'userDetails.image2': imageUrl2,
            'userDetails.image3': imageUrl3,
            'userDetails.image4': imageUrl4,
            'userDetails.image5': imageUrl5,
            'userDetails.religionType': data?.religionType ? data?.religionType : null,
            'userDetails.SelectionOne': data?.selection1,
          }).then(() => {
            setUploading(false)
            Toast.show('Succesfully completed!', Toast.SHORT)
            navigation.navigate('Home');
            refereshForm()
          })
        // setUploading(false)
      ...
      if (!smokeData) {
        Toast.show("Please select do you smoke!", Toast.SHORT);
      }
      else if (!vapeData) {
        Toast.show("Please select do you vape!", Toast.SHORT);
      }
      else if (!marijuanaData) {
        Toast.show("Please select do you marijuana!", Toast.SHORT);
      }
      else if (!drugsData) {
        Toast.show("Please select do you drugs!", Toast.SHORT);
      }
      else if (!drinkData) {
