...
export const PositionTestimonial = [
  {
    id: '1',
    name: '#CEO',
  },
...
const QuestionOccupationScreen = ({ navigation, route }) => {
  ...
  const [typeTestimonial, setTypeTestimonial] = useState(CompanyBusiness);
  const [temptypeTestimonial, setTempTypeTestimonial] = useState(CompanyBusiness);
  ...
  const onInterestScreen = () => {
    ...
    if (selectedCategoryIndex !== null && selectedPositionIndex !== null) {
      const CompanyType = CompanyBusiness[selectedCategoryIndex].name;
      ...
      navigation.navigate('QuestionDietScreen', update)
    }
    else {
      if (selectedCategoryIndex == null) {
        Toast.show("Please select company type!", Toast.SHORT);
      }
      else if (selectedPositionIndex == null) {
        Toast.show("Please select company position!", Toast.SHORT);
      }
    }
  }
  // console.log(route?.params);
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={{
            ...
            <View style={styles.Options}>
              <Text style={{ color: COLORS.black, fontSize: 12 }}>(Optionals)</Text>
            </View>
            <View style={{
              alignItems: 'center',
            }}>
