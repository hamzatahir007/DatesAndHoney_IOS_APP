import { Image, SafeAreaView,Platform, StatusBar, StyleSheet, ScrollView, Text, View, ToastAndroid, TouchableOpacity } from 'react-native'
...
const QuestionDoyouSmoke = ({ navigation, route }) => {
  ...
  const onPartnerAge = async () => {
    ...
    if (smoke && vape && marijauna && drinks && drugs) {
      ...
      if (!smoke) {
        Toast.show("Please select smoke type!", Toast.SHORT);
      }
      else if (!vape) {
        Toast.show("Please select vape type!", Toast.SHORT);
      }
      else if (!marijauna) {
        Toast.show("Please select marijauna type!", Toast.SHORT);
      }
      else if (!drinks) {
        Toast.show("Please select drinks type!", Toast.SHORT);
      }
      else if (!drugs) {
        Toast.show("Please select drugs type!", Toast.SHORT);
      }
    }
  }
  ...
  const onQuestionHeightPartnerScreen = () => {
    if (height) {
      ...
      Toast.show("Please select your Height!", Toast.SHORT);
    }
  }
