import { Image, SafeAreaView, StatusBar, StyleSheet, Text, View, TextInput, ToastAndroid, Dimensions, Platform } from 'react-native'
...
const LoginWithOTPScreen = ({ navigation, route }) => {
  ...
  const getCodeHandler = () => {
    if (value == '') {
      Toast.show("Code cannot be empty", Toast.SHORT);
      return false;
    }
    else {
      onConfirmPressed();
  ...
  const onConfirmPressed = async () => {
    ...
    try {
      ...
      Toast.show('User Sign In Successfully', Toast.SHORT);
    }
    catch (error) {
      setUploading(false);
  ...
  function onAuthStateChanged(user) {
    ...
    if (user) {
      ...
      Toast.show('User Sign In Succesfully', Toast.SHORT);
      navigation.navigate('NameScreen')
    }
  }
            ...
            renderCell={({ index, symbol, isFocused }) => (
              <Text
                ...
                style={[
                  ...
                  !symbol && styles.placeholderCell, // Apply placeholder style if no symbol
                ]}
...
const styles = StyleSheet.create({
  ...
  codeFieldRoot: { marginTop: 20, borderWidth: 0 },
  cell: {
    color: COLORS.black,
    width: 40,
    height: 40,
    ...
    // Add borderBottomWidth for iOS only
