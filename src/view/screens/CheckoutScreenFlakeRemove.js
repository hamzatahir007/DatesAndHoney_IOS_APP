...
const CheckoutScreenFlakeRemove = ({ navigation, route }) => {
    const { data } = route.params;
    // const FlakeBill = data;
    const SelectedEvent = useSelector(selectEvents);
    const SelectedFlakeBill = useSelector(selectFlakesBill);
    ...
    const [FlakeBill, setFlakeBill] = useState(data);
                        ...
                        await firestore()
                            ...
                            .then(async () => {
                                ...
                                const userRef = await firestore().collection('Users')
                                    .doc(user.uid)
                                userRef.update({
                                    'userDetails.Flake': user?.Flake - number,
    ...
    const updateTicketPrices = async () => {
        ...
        try {
            const ref = firestore().collection('Package').doc('789')
            ...
            if (FlakeBill) {
                ...
                let FlakeInsurance = refdoc?.data()?.AdditionalPackages[4]
                if (
                    user?.FlakeInsurance &&
                    ...
                    const discountPrice = parseFloat((FlakeBill?.TotalPrice * FlakeInsurance?.discount / 100)).toFixed(2);
                    ...
                    // console.log('===>', FlakeInsurance?.discount);
                    setFlakeBill(update)
                    dispatch(FlakesBill(update))
                    // console.log(SelectedFlakeBill, '==>');
                }
                else {
                    const update = {
                        ...FlakeBill,
                                    ...
                                    {FlakeBill?.discountPrice &&
                                        <View style={{
                                            ...
                                            <View style={{
                                                ...
                                                <Text style={{ fontSize: 12, color: COLORS.gray, marginRight: 5 }}>Flake Insurance(75%)</Text>
                                                {/* <SVGNotify width={15} height={15} onPress={() => setTaxes(true)} /> */
                                                    // console.log(FlakeBill, '===>')
                                                }
                                            </View>
...
export default CheckoutScreenFlakeRemove
