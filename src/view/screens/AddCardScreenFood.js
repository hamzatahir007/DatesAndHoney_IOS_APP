import { TouchableOpacity, Platform,SafeAreaView, Image, StatusBar, StyleSheet, Text, View, ScrollView, ToastAndroid, Dimensions, Alert, ActivityIndicator } from 'react-native'
...
import { PaymentCardDetails, selectPaymentCardDetails, selectPaymentCards, selectPaymentMethod, selectUser } from '../../../redux/reducers/Reducers';
...
const AddCardScreenFood = ({ navigation, route }) => {
        ...
        if (!cardName || !cardHolder || cardHolder?.length < 16 || cardHolder?.length > 16 || !ExpMonth || ExpMonth > 12 || !ExpYear || ExpYear?.length > 2 || !cvc) {
            if (!cardName) {
                ...
                if (!cardHolder) {
                    ...
                    Toast.show("Card number must be in 16 digits!!", Toast.SHORT);
                    setInputCardHolder(true)
                }
            }
            else if (!ExpMonth) {
                Toast.show("Please enter card expiry month!", Toast.SHORT);
                setInputExpMonth(true)
            }
            else if (ExpMonth > 12) {
                Toast.show("Please enter valid expiry month which is 12 or less!", Toast.SHORT);
                setInputExpMonth(true)
            }
            else if (!ExpYear) {
                Toast.show("Please enter card expiry year!", Toast.SHORT);
                setInputExpYear(true)
            }
            else if (ExpYear?.length > 2) {
                Toast.show("Please enter card valid expiry year in two digits!", Toast.SHORT);
                setInputExpYear(true)
            }
            else if (!cvc) {
                Toast.show("Please enter card cvc number!", Toast.SHORT);
                setInputCvc(true)
            }
        }
        else {
                                ...
                                <View style={{
                                    ...
                                    <View style={{
                                        ...
                                        <View style={{
                                            ...
                                            {allCardDetail?.last4 ?
                                                <View>
                                                    <Text>{cardHolder ? cardHolder : `************${allCardDetail?.last4}`}</Text>
                                                </View>
                                                :
                                                <View>
...
export default AddCardScreenFood
