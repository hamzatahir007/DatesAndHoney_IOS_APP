...
const CheckoutScreenConcriege = ({ navigation, route }) => {
    const { data } = route.params;
    const [ConciergeRequestData, setConciergeRequestData] = useState(data)
            ...
            if (totalBalance >= price) {
                try {
                    ...
                    const requestData = {
                        ...
                        mediatorName: ConciergeRequestData?.mediatorName,
                MonthlyRatesType: ConciergeRequestData?.MonthlyRatesType,
                        // mediatorid: ConciergeRequestData?.mediatorid,
                ...
                MonthlyRatesType: requestData?.MonthlyRatesType,
                                DateAndHoneyFee: requestData?.DateAndHoneyFee,
                                transactionFee: requestData?.transactionFee,
                                chargeid: requestData?.chargeid ? requestData?.chargeid : null,
                                date: new Date().toString(),
                                // You can update other fields here as well
                            };
                    ...
                    setShowModalContent({
                        ...showModalContent,
                        title: 'Request Sent!',
                        descrition: `Your request has been sent to ${requestData?.mediatorName}, the Match Coordinator. Please wait for their response. Your amount of ${requestData.price} has been deducted when the match coordinator accepts your request.`,
                        type: 'Success',
                    })
                    setShowModal(true)
    ...
    const updateTicketPrices = () => {
        if (ConciergeRequestData) {
            ...
            // console.log(update);
            setConciergeRequestData(update)
...
export default CheckoutScreenConcriege
