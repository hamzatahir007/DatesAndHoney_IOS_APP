import { Animated, Dimensions, StyleSheet, Text, TouchableOpacity, View, SafeAreaView, Image, TextInput, ScrollView, ActivityIndicator, StatusBar, ToastAndroid ,Platform} from 'react-native';
...
const EventTickets = ({ navigation, route }) => {
  ...
  const handleDecrement = (item, index) => {
    ...
    if (item?.totalTickets > 0) {
      ...
      if (item) {
        ...
        if (item?.qty < 1) {
          Toast.show('Please add Tickets Qty', Toast.SHORT)
        }
        else {
          let Data = new Object();
        ...
        Toast.show('Please Select Your Tickets', Toast.SHORT)
      }
    }
    else {
      Toast.show('Tickets Out Off the Strock', Toast.SHORT)
    }
  }
  ...
  const AddtoCard = (item) => {
    if (item.qty < 1) {
      Toast.show('Please select Tickets Qty', Toast.SHORT)
    }
    // return
  ...
  const handleTicket = () => {
    ...
    if (addToCard) {
      if (addToCard?.Tickets?.qty < 1) {
        ...
        if (PaymentCards?.length > 0) {
          navigation.navigate('CheckoutScreenEvent')
        }
        else {
    ...
    // }
    // else {
    //   Toast.show("Please select your ticket to continue!", Toast.SHORT);
    // }
    // setValueIndex(index)
    // setEvents(viewPage);
  };
