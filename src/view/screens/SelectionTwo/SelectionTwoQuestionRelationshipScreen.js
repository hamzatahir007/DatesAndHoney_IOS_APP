...
const SelectionTwoQuestionRelationshipScreen = ({ navigation, route }) => {
  ...
  const onReligionScreen = async () => {
    ...
    if (typeOfRealtionship?.length > 0 && selectedCoupleIndex !== null) {
      ...
      // console.log(update);
      // return
      navigation.navigate('SelectionTwoQuestionWantKidsScreen', {
        RelationshipType: typeOfRealtionship,
        CoupleOption: selectitem,
        selection2: 1,
      })
      ...
      if (selectedCoupleIndex == null) {
        Toast.show("Please select couple options!", Toast.SHORT);
      }
      else {
        Toast.show("Please select your relationship type you are looking for!", Toast.SHORT);
      }
    }
  }
  const onSkip = async () => {
