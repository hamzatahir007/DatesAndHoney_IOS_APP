import { SafeAreaView, StatusBar, StyleSheet, Text, View, Image, Platform, useWindowDimensions, TouchableOpacity, ActivityIndicator, Dimensions, Modal, ToastAndroid, ScrollView, Pressable } from 'react-native';
...
const ChatingScreen = ({ navigation, route }) => {
        ...
        try {
            ...
            await Ref_firestore.update({ PrivateChat: UpdatePrivateChat })
                .then(() => {
                    setUploading(false)
                    navigation.goBack();
                    // Toast.show('User removed from your matches!', Toast.SHORT)
                ...
                .onSnapshot((querySnap) => {
                    ...
                    if (querySnap?.data()) {
                        querySnap?.data()?.Proposals?.map(item => {
                            ...
                            if ((item?.sentBy == uid || item?.sentTo == uid) && (item?.sentTo == Currentuser?.uid || item?.sentBy == Currentuser?.uid)) {
                                Proposals.push(item);
    ...
    const onSendPorposal = () => {
        ...
        if (!dates && !tempDates) {
            // console.log('aklxjn');
            Toast.show("Please select proposal Date!", Toast.SHORT);
        }
        else if (!time) {
            Toast.show("Please select proposal time!", Toast.SHORT);
        }
        else if (!description) {
            Toast.show("Please add proposal description!", Toast.SHORT);
        }
        else if (!pin) {
            Toast.show("Please add your location!", Toast.SHORT);
        }
        else if (!category) {
            Toast.show("Please your time again!", Toast.SHORT);
        }
        else {
            let mymsg = null;
    ...
    const YouArrived = (item, index) => {
        ...
        if (!test?.length == 0 && !chatUser?.length == 0) {
            ...
            //             Toast.show('You are arrived!', Toast.SHORT)
            //         });
            // }
            // catch (e) {
            //     console.log('Error', e);
    ...
    const YouNotArrived = (item, index) => {
        ...
        //             // navigation.goBack();
        //             // console.log('Event deleted!');
        //             Toast.show('Date cancled!', Toast.SHORT)
        //             const userRef = firestore().collection('Users')
        //                 .doc(Currentuser.uid)
        //             userRef.update({
        //                 'userDetails.Flake': + 1,
    ...
    const PNoArrived = (item, index) => {
        ...
        if (ctime >= NProposaltime) {
            ...
            Toast.show(`Please wait for ${NProposaltime - ctime} hr to perform any action.`, Toast.SHORT)
            // console.log(item);
        }
    };
    ...
    const RejectProposal = (uid) => {
        ...
        userRef.delete()
            .then(() => {
                ...
                // navigation.goBack();
                // console.log('Event deleted!');
                Toast.show('Proposal Delete!', Toast.SHORT)
            });
    }
    ...
    const SendToChat = (url) => {
        ...
        if (url) {
            ...
            Toast.show("Network issue try again!", Toast.SHORT);
        }
    }
    ...
    const CancleDate = () => {
        ...
        try {
            firestore()
                ...
                .then(() => {
                    Toast.show('Date canceled!', Toast.SHORT)
                    setLoading(false)
                    setModal(false)
                })
            firestore()
    ...
    const OnSetLocation = () => {
        if (region) {
            ...
            Toast.show("Please select location first!", Toast.SHORT);
        }
    }
    ...
    const handleSubmission = async (modalType) => {
        if (!reason || reason == '') {
            Toast.show("Please enter reasone!", Toast.SHORT);
        }
        else {
            setLoading(true)
            if (modalType == 'unmatch') {
                ...
                try {
                    ...
                    if (refdoc?.exists) {
                        ...
                        if (index != -1) {
                            OnDeleteUser()
                            Toast.show(`${userName} removed from your matches successfully`, Toast.SHORT);
                ...
                try {
                    ...
                    if (mydocGet?.exists) {
                        ...
                        if (index) {
                            ...
                            setLoading(false)
                            // return
                            Toast.show(`${userName} Blocked successfully`, Toast.SHORT);
                            OnDeleteUser()
                            SendPushNotify(uid, Currentuser, `Hase been remove you from match!`, 'Matched')
                            setModalVisible(false)
                            setReason(null)
            ...
            if (modalType == 'report') {
                try {
                    ...
                    if (refdoc?.exists) {
                        ...
                        const index = Report.findIndex(item => item?.from == Currentuser?.uid && item.to == uid);
                        if (index != -1) {
                            Toast.show(`Report already sended to D&H Team!`, Toast.SHORT);
                            setModalVisible(false)
                            setReason(null)
                            setLoading(false)
                            return
                            ...
                            await ref?.set(
                                ...
                                .then(() => {
                                    Toast.show(`Your Report for ${userName} hase Sended to D&H teams!`, Toast.SHORT);
            ...
            <View style={styles.container}>
                ...
                >
                    ...
                    <Modal
                        ...
                        <View style={{ alignItems: 'center', flex: 1, top: 10 }}>
                        ...
                        <View style={{
                            ...
                            <View>
                                ...
                                <View style={{
                                    ...
                                    <View style={{ marginTop: 10, width: windowWidth / 1.1 }}>
                                    ...
                                    <View style={{ marginTop: 10, width: windowWidth / 1.1 }}>
                                        ...
                                        <View style={{
                                            ...
                                            {location ?
                                                ...
                                                >
                                                    <Text numberOfLines={1} style={{ color: COLORS.black }}>{location}</Text>
                                                </TouchableOpacity>
                                                :
                                                <TouchableOpacity
                                                    style={{
                                                ...
                                                >
                                                    <Text style={{ color: COLORS.gray }}>Select Location</Text>
                                                </TouchableOpacity>
                                            }
                                            {/* <TextInput
                                                editable={false}
                                                ...
                                                numberOfLines={1}  // Set to 1 to limit the number of lines
                                                ellipsizeMode="tail"
                                            /> */}
                                            <Image source={require('../../assets/selectdate.png')} resizeMode='contain' style={{
                                                width: 25,
