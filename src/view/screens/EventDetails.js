...
const EventDetails = ({ navigation, route }) => {
  ...
  const onSuccess = (e) => {
    if (e) {
      ...
      Toast.show("Please scan your Doc first!", Toast.SHORT);
    }
  }
  const startScan = () => {
  ...
  const VerifyScanDoc = (props) => {
    if (!props || props == null) {
      Toast.show("Doc scan error please try again!", Toast.SHORT);
      // navigation.goBack()
    }
    else {
      // console.log('QR Result==> :', props);
      Toast.show("Doc scaned now buy tickets!", Toast.SHORT);
  ...
  const OnBuyTickets = () => {
    ...
    if (!showID.enable) {
      ...
      Toast.show("Please enter DL or ID to continue", Toast.SHORT);
    }
    else {
